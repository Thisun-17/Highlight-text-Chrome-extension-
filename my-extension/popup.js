...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      ...
      if (url.protocol === 'chrome:' || url.protocol === 'chrome-extension:' || 
        ...
        function handleSelectedTextResponse(response) {
          if (response && response.selectedText) {            saveToStorage('text', {
              text: response.selectedText,
              pageUrl: response.pageUrl,
              pageTitle: response.pageTitle
            });
            statusDiv.textContent = 'Text saved! (Only first selection will be saved)';
          } else {
            ...
            try {
              chrome.tabs.sendMessage(tabs[0].id, { action: "highlightText", color: highlightColor }, function(highlightResponse) {
                ...
                if (highlightResponse && highlightResponse.success) {
                  ...
                  statusDiv.textContent = 'Text highlighted and saved! (Only first selection will be saved)';
                } else {
      ...
      try {
        chrome.tabs.sendMessage(tabs[0].id, {action: "getImage"}, function(response) {
          if (chrome.runtime.lastError) {
            ...
            statusDiv.textContent = 'Image saved! (Only first selection will be saved)';
          } else {
  ...
  saveArticleBtn.addEventListener('click', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
      ...
      // Unlike other functions, articles can be saved from any page      const url = tabs[0].url;
      const title = tabs[0].title;
      saveToStorage('article', {url: url, title: title});
      statusDiv.textContent = 'Article saved! (Only first selection will be saved)';
    });
  });
  ...
  function saveToStorage(type, content) {
    chrome.storage.local.get(['savedItems'], function(result) {
      ...
      if (recentItemWithSameContent) {
        ...
        statusDiv.textContent = 'Duplicate content not saved. Only first selection is saved.';
        return;
      }
